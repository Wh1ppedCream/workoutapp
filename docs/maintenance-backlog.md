...
The latest committed work is `05476dd` on `origin/updates/backlog`, based on
`f14eee0` from `origin/master`. The cloud-content boundary batch and the
active-workout durability batch both passed their focused coverage; the full
suite passes 200 Flutter tests. The diagnostics relay is implemented but
disabled.
...
7. **Use one duration model everywhere.**
...
8. **Audit the exercise catalog for safety and data quality.**
...
9. **Localize catalog entities, not only interface messages.**
...
10. **Strengthen localization enforcement and language review.**
...
11. **Strengthen body-measurement validation and lifecycle.**
...
12. **Decide the offline-media experience.**
...
13. **Continue exercise and anatomy media.**
...
14. **Finish production media when a custom domain is ready.**
...
16. **Align CI and branch protection with feature-branch development.**
...
18. **Review visible incomplete navigation and plan complete implementations.**
...
21. **Restore cardio and stretch as a complete vertical feature.**
...
22. **Remove obsolete badge and active-session implementations.**
...
24. **Decompose non-database hotspots alongside feature work.**
...
25. **Standardize safe error handling and recovery UI.**
...
26. **Create performance and resource baselines.**
...
27. **Perform a controlled Flutter and dependency upgrade.**
...
28. **Define the supported-platform matrix.**
...
29. **Remove tracked build artifacts and repair repository hygiene.**
...
30. **Prune merged branches and document branch ownership.**
...
31. **Provision the diagnostics relay only when remote diagnostics become a
...
32. **Rename the internal Dart package only as a dedicated migration.**
