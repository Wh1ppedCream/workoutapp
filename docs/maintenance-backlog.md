...
`updates/backlog` contains the active release-preparation work and is 15 commits
...
1. The production workflow passes `TONOS_ENVIRONMENT=production`, but runtime
   code never reads that define. A fresh production build therefore still uses
   the bundled development default unless a saved preference overrides it.
2. Persisted dates mix local ISO strings, UTC ISO strings, and UTC epoch values.
   Several range queries compare ISO text directly, creating daylight-saving
   and time-zone-change risks for workout, measurement, and nutrition history.
3. Database export coverage is a hand-maintained table list. Newer user-owned
   allocation overrides and record-event tables are absent without a declared
   derived/app-owned exclusion policy or restore-equivalence contract.
...
7. **Make release environment selection explicit and testable.**
   Either consume an allowlisted build-time environment define or generate a
   release-specific bundled content configuration. Keep developer/custom URLs
   out of ordinary release UI, define precedence over saved preferences, and add
   contracts proving development builds cannot silently become production and
   signed production builds cannot silently remain on development.

8. **Define backup scope and prove restore equivalence.**
...
9. **Normalize timestamp and local-calendar semantics.**
...
10. **Complete health-product legal and store-readiness review.**
...
11. **Finish production media when a custom domain is ready.**
...
12. **Complete native-speaker localization review.**
...
13. **Close localization boundaries outside widget literals.**
...
14. **Localize catalog entities using stable codes.**
...
15. **Decide the offline-first media experience.**
...
16. **Expand device, visual-regression, and accessibility coverage.**
...
17. **Align CI, tooling checks, and branch protection with feature branches.**
...
18. **Establish release versioning and store-delivery governance.**
...
19. **Version and migrate the preference schema.**
...
20. **Make optional hardware and platform permissions explicit.**
...
21. **Create a complete licensing, attribution, and data-provenance inventory.**
...
22. **Complete the local-data reset and retention lifecycle.**
...
23. **Decide the local sensitive-data security model.**
...
24. **Choose and wire the production food-catalog strategy.**
...
25. **Complete nutrition as a coherent product stream.**
...
26. **Restore cardio and stretch as complete vertical features.**
...
27. **Continue exercise and anatomy media.**
...
28. **Remove obsolete, fabricated, and unreachable implementations.**
...
29. **Continue focused database decomposition alongside related work.**
...
30. **Decompose other oversized feature files alongside feature work.**
...
31. **Define privacy-safe release logging.**
...
32. **Create performance, battery, and storage baselines.**
...
33. **Perform a controlled Flutter and dependency upgrade.**
...
34. **Define and enforce the supported-platform matrix.**
...
35. **Remove tracked artifacts and repair repository hygiene.**
...
36. **Prune branches after preserving unique work.**
    `updates/backlog` is the active branch and is 15 commits ahead of `master`.
...
37. **Make content status documentation generated or mechanically verified.**
...
38. **Rehearse release-candidate installation, upgrade, and rollback.**
...
39. **Define user support and feedback without remote diagnostics.**
...
40. **Provision the diagnostics relay only when remote diagnostics are needed.**
...
41. **Rename the internal Dart package only as a dedicated migration.**
...
42. **Implement the planned durable media library when scale requires it.**
...
The strongest near-term sequence is: **finish catalog verification -> resolve
release scope -> audit catalog safety -> retire Train2 -> rework the Exercise
