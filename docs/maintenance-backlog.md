...
The latest committed work is `52a054d` on `origin/updates/backlog`, based on
`f14eee0` from `origin/master`. The cloud-content boundary batch passed its 15
focused tests and the full suite of 188 Flutter tests. The diagnostics relay is
...
2. **Make local-data backup and export privacy explicit.**
   Decide whether Android cloud backup and device transfer may include the
   health database, then encode that decision with `allowBackup`, backup rules,
   and a manifest contract. Explicitly disallow cleartext release traffic.
   Warn that JSON exports contain sensitive plaintext data, set import size
   limits, validate integrity before mutation, guarantee rollback, and decide
   whether an encrypted export option is needed.

3. **Make active-workout durability failures visible and recoverable.**
   Draft save, restore, and automatic plan-progression failures currently fall
   back to debug output in important paths. Add bounded retries, privacy-safe
   local diagnostics, and a non-disruptive user warning when resume protection
   or progression could not be saved. Preserve the completed workout even if
   progression fails, and test process death, write failure, retry, and restore.

4. **Retire the alternative Train hub.**
...
6. **Improve starter-weight calibration.**
...
7. **Validate generated plans and automatic progression as a product system.**
...
8. **Use one duration model everywhere.**
...
9. **Audit the exercise catalog for safety and data quality.**
...
10. **Localize catalog entities, not only interface messages.**
...
11. **Strengthen localization enforcement and language review.**
...
12. **Strengthen body-measurement validation and lifecycle.**
...
13. **Decide the offline-media experience.**
...
14. **Continue exercise and anatomy media.**
...
15. **Finish production media when a custom domain is ready.**
...
17. **Align CI and branch protection with feature-branch development.**
...
19. **Review visible incomplete navigation and plan complete implementations.**
...
20. **Choose and wire the production food-catalog strategy.**
...
21. **Complete nutrition as a coherent product stream.**
...
22. **Restore cardio and stretch as a complete vertical feature.**
...
23. **Remove obsolete badge and active-session implementations.**
...
25. **Decompose non-database hotspots alongside feature work.**
...
26. **Standardize safe error handling and recovery UI.**
...
27. **Create performance and resource baselines.**
...
29. **Define the supported-platform matrix.**
...
30. **Remove tracked build artifacts and repair repository hygiene.**
...
31. **Prune merged branches and document branch ownership.**
...
32. **Provision the diagnostics relay only when remote diagnostics become a
...
33. **Rename the internal Dart package only as a dedicated migration.**
...
backup and export privacy -> make workout durability recoverable -> retire
Train2 -> rework the Exercise Editor -> implement starter feedback**.
