...
Future<void> _writeUploadScript({
  ...
  final normalizedManifestObject = _normalizeRemotePath(manifestObject);
  final lines = <String>[
  ...
  void addUploadCommand({
    ...
    lines
      ..add(
        'wrangler r2 object put ${_psQuote(remotePath)} '
        '--file ${_psQuote(localFile)} '
        '--content-type ${_psQuote(contentType)} '
        '--remote',
      )
      ..add(
        'if (\$LASTEXITCODE -ne 0) { throw "Wrangler upload failed for '
        '${_psQuote(remotePath)} (exit code \$LASTEXITCODE)." }',
      );
  }
  ...
  for (final exercise in sourceExercises.whereType<Map>()) {
    ...
    for (final rawAsset in assets.whereType<Map>()) {
      ...
      addUploadCommand(
        remotePath: '$bucket/$remotePath',
        localFile: localFile,
        contentType: _contentTypeFor(remotePath),
      );
    }
  }

  addUploadCommand(
    remotePath: '$bucket/$normalizedManifestObject',
...
String _normalizeRemotePath(String path) => path.replaceAll('\\', '/');
