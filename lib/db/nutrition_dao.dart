...
Future<int> upsertFoodPortion(FoodPortion p) async {
  ...
  Future<List<RecipeIngredient>> getRecipeIngredients(int recipeId) async {
    final rows =
        await db.query('recipe_ingredients', where: 'recipe_id = ?', whereArgs: [recipeId]);
  ...
  Future<int> addDiaryFood({
    ...
    final resolvedGrams =
        loggedGrams ?? gramsOverride ?? await _resolveFoodGrams(foodId, portionId, quantity);
  ...
  Future<void> updateDiaryEntry(DiaryEntry e) async {
    ...
    final oldRows =
        await db.query('diary_entries', where: 'id = ?', whereArgs: [e.id], limit: 1);
  ...
  Future<void> deleteDiaryEntry(int id,
      {required int profileId, required DateTime date}) async {
...
Future<List<DiaryEntryWithItem>> getDiaryEntriesWithItemsForDate(
  ...
  Future<DayTotals> getDayTotals(int profileId, DateTime date) async {
    ...
    final rows = await db.query('day_totals_cache',
        where: 'profile_id = ? AND date = ?', whereArgs: [profileId, ymd], limit: 1);
  ...
  Future<double?> _resolveIngredientGrams(RecipeIngredient ing) async {
    ...
    if (ing.portionId != null && ing.quantity != null) {
      final rows = await db.query('food_portions',
          where: 'id = ?', whereArgs: [ing.portionId], limit: 1);
      if (rows.isNotEmpty) {
        final p = FoodPortion.fromMap(rows.first);
  ...
  Future<int> insertCustomFood({
    ...
    return await db.transaction<int>((txn) async {
      ...
      int? brandId;
      if (brandName != null) {
        await txn.insert('brands', {'name': brandName},
            conflictAlgorithm: ConflictAlgorithm.ignore);
  ...
  Future<Map<String, int>> _codeToId(Set<String> codes) async {
    ...
    final byCode = <String, int>{
      for (final r in codeRows) (r['code'] as String).toUpperCase(): (r['id'] as int),
    };
  ...
  ''', wanted.toList());
    ...
    for (final orig in codes) {
      ...
      final id = byCode[dbCode] ?? byCode[uOrig] ?? byAlias[dbCode] ?? byAlias[uOrig];
  ...
  await db.transaction((txn) async {
    ...
    for (final e in normalized.entries) {
      ...
      await txn.insert(
        'food_nutrient_values',
        {'food_id': foodId, 'nutrient_id': nid, 'amount': e.value, 'basis': 'per_100g'},
        conflictAlgorithm: ConflictAlgorithm.replace,
      );
      await txn.insert(
        'food_nutrients',
        {'food_id': foodId, 'nutrient_id': nid, 'amount_per_100g': e.value},
        conflictAlgorithm: ConflictAlgorithm.replace,
      );
    }
  });
...
Future<Map<String, double>> mapLabelsToCodes(
  Database db, Map<String, dynamic> labelToValue) async {
  final aliasMap = await _buildAliasToCodeMap(db);

  double nums(dynamic x) => (x is num) ? x.toDouble() : double.tryParse('$x') ?? 0.0;
  bool containsKj(String s) => RegExp(r'\b(kj|kilojoule|kilojoules)\b', caseSensitive: false).hasMatch(s);
  ...
  for (final e in labelToValue.entries) {
    ...
    final norm = _normalizeLabel(leafRaw); // strips " (unit)" → e.g., "energy"

    final resolved = aliasMap[norm];
    if (resolved == null) continue;

    var code = resolved; // e.g., "KCAL", "PROTEIN_G", "ENERGY_KCAL", "KJ", ...
  ...
  Future<void> savePer100gFromLabelPayload(int foodId, Map<String, dynamic> payload) async {
    ...
    final mappedNorm = {for (final e in mapped.entries) _canonCode(e.key): e.value};
    final baseNorm = {for (final e in base.entries) _canonCode(e.key): e.value};

    // Merge (explicit base wins if there’s conflict)
    final codeToAmount = <String, double>{}..addAll(mappedNorm)..addAll(baseNorm);
...
Future<void> replacePortions(int foodId, List<FoodPortion> portions) async {
  ...
  Future<int?> _ensureBrandTx(DatabaseExecutor ex, String? name) async {
    ...
    return Sqflite.firstIntValue(await ex.rawQuery(
      'SELECT id FROM brands WHERE lower(name) = lower(?) LIMIT 1', [n]));
  }
  ...
  Future<int?> _ensureSourceTx(DatabaseExecutor ex, String? name) async {
    ...
    return Sqflite.firstIntValue(await ex.rawQuery(
      'SELECT id FROM sources WHERE lower(name) = lower(?) LIMIT 1', [n]));
  }
  ...
  Future<int?> _ensureCategoryTx(DatabaseExecutor ex, String? name) async {
    ...
    return Sqflite.firstIntValue(await ex.rawQuery(
      'SELECT id FROM categories WHERE lower(name) = lower(?) LIMIT 1', [n]));
  }
  ...
  Map<String, double> _computeMacroSnapshot(
    ...
    final kcal = (kcalDirect > 0) ? kcalDirect : 4 * protein + 4 * carbs + 9 * fat;
  ...
  Future<List<int>> _recipeIdsUsingFood(int foodId) async {
    final rows =
        await db.rawQuery('SELECT DISTINCT recipe_id FROM recipe_ingredients WHERE food_id = ?', [foodId]);
  ...
  Future<List<DiaryEntry>> getEntriesByTag({
    ...
    final where =
        StringBuffer('e.profile_id = ? AND det.tag = ? AND e.is_deleted = 0');
  ...
  Future<Map<String, double>> getDayMicros(
    ...
    final out = <String, double>{};
    origToCanon.forEach((orig, canon) { out[orig] = totalsByCanon[canon] ?? 0.0; });
    return out;
  }
