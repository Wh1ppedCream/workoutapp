...
// Inside the same file, below your imports, define:
class HistoryRecord {
  final DateTime date;
  final int sessionId;
      ...
      child: Material(
        ...
        child: Column(
          children: [
            ...
            // Tab Bar (no controller parameter required now)
            const TabBar(
              tabs: [
                Tab(text: 'Details'),
                Tab(text: 'Metrics'),
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      children: const [
                        Expanded(child: Text('Reps')), Expanded(child: Text('1RM')), Expanded(child: Text('Volume'))
                      ],
                    ),
                    const Divider(),
                    Expanded(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Row(
                            children: [
                              ...
                              Expanded(child: Text(r.isErm ? '${r.oneErm.toStringAsFixed(1)} (ERM)' : r.rmValue.toStringAsFixed(1))),
...
/// Loads the last up-to-10 weight-exercise instances for this definition.
Future<List<HistoryRecord>> _loadHistory() async {
  final repo = AppRepository();
  final allSessions = await repo.fetchWorkoutSessions(); // newest first
  final records = <HistoryRecord>[];
  for (var session in allSessions) {
    // fetch all exercises in that session
    final exRows = await repo.fetchExercises(session.id);
    for (var exRow in exRows) {
      if (exRow['type'] == 'weight'
          && exRow['exercise_def_id'] == widget.defId) {
        // detailed model
        final we = await repo.fetchDetailedExercise(exRow['id'] as int);
...
// Then in your TabBarView, replace the Records placeholder with:
        ...
        children: [
          SizedBox(
            ...
            child: BarChart(
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        ...
                        // Pass the full TitleMeta into SideTitleWidget
              ...
              // Use the new animation parameters:
