...
static const double _chipW = 195;        // NEW: fixed width for each entry chip
  ...
  void didChangeDependencies() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (p.isToday) {
        ...
        _scroll.jumpTo(targetOffset.clamp(0, maxExtent).toDouble()); // ← cast to double
      }
    });
  }
  ...
  switch (m) {
    case MealType.breakfast: return cs.primaryContainer;
    case MealType.lunch:     return cs.tertiaryContainer;
    case MealType.dinner:    return cs.secondaryContainer;
    case MealType.snack:     return cs.errorContainer;
  }
}

Color _mealOnFill(MealType m, ColorScheme cs) {
  switch (m) {
    case MealType.breakfast: return cs.onPrimaryContainer;
    case MealType.lunch:     return cs.onTertiaryContainer;
    case MealType.dinner:    return cs.onSecondaryContainer;
    case MealType.snack:     return cs.onErrorContainer;
  }
}
      ...
      body: Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              children: [
                Expanded(child: _MiniStat(label: 'Calories', value: '$kcal / $kcalTgt kcal')),
                const SizedBox(width: 8),
                Expanded(child: _MiniStat(label: 'Fat',      value: '$fat / $fatTgt g')),
                const SizedBox(width: 8),
                Expanded(child: _MiniStat(label: 'Protein',  value: '$pro / $proTgt g')),
                const SizedBox(width: 8),
                Expanded(child: _MiniStat(label: 'Carbs',    value: '$carb / $carbTgt g')),
              ],
            ),
          ),
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: Padding(
                            ...
                            child: Text(_hourLabel(h), style: Theme.of(context).textTheme.bodySmall),
                          ),
                        ),
                  ...
                  nowLine.add(Positioned(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(child: Container(height: 2, color: cs.primary)),
                      ],
                    ),
                  ));
                }
...
buckets.forEach((key, entries) {
  ...
  final y = (bucketMinutes / (24 * 60)) * totalHeight - (_chipH / 2);
                ...
                return Scrollbar(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: ConstrainedBox(
                      constraints: BoxConstraints(minHeight: totalHeight, maxHeight: totalHeight),
          ...
          children: [
            ListTile(
              ...
              onTap: () async {
                ...
                messenger.showSnackBar(const SnackBar(content: Text('Edit flow not wired yet')));
              },
            ),
            ListTile(
...
Widget build(BuildContext context) {
  ...
  return SizedBox(
    ...
    child: Container(
      padding: const EdgeInsets.symmetric(horizontal: 6), // no vertical padding
      decoration: BoxDecoration(
...
Widget build(BuildContext context) {
  ...
  return Material(
    ...
    child: MediaQuery( // <- prevent system text scaling from breaking fixed height
      data: MediaQuery.of(context).copyWith(
        textScaler: const TextScaler.linear(1.0),
      ),
      child: InkWell(
