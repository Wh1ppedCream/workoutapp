...
class _BodyPartMuscleMappingScreenState
  ...
  Future<void> _loadLookups() async {
    final bodyPartsFuture = _repo.fetchAllBodyPartsFull();
    final musclesFuture = _repo.fetchAllMusclesFull();
    final bps = await bodyPartsFuture;
    final ms = await musclesFuture;
    if (!mounted) return;
    setState(() {
      _bodyParts = bps;
      _muscles = ms;
  ...
  Future<void> _loadMappings(int bodypartId) async {
    final loadGeneration = ++_mappingLoadGeneration;
    final rows = await _repo.fetchMusclesForBodyPart(bodypartId);
    if (!mounted || loadGeneration != _mappingLoadGeneration) return;
    setState(() {
